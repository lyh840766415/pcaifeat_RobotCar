...
MODEL_PATH = '/home/lyh/lab/pcaifeat_RobotCar/log/train_save_256/model_00900000.ckpt'
BATCH_NUM_QUERIES = 2
EPOCH = 100
...
def main():
	...
	with tf.Session(config=config) as sess:
		...
		train_writer = tf.summary.FileWriter(os.path.join(LOG_DIR, 'train_save_256'),sess.graph)
		...
		for ep in range(EPOCH):
			...
			for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
				...
				if step%3000 == 0:
					save_path = model_saver.save(sess, os.path.join(LOG_DIR,"train_save_256", "model_%08d.ckpt"%(step)))
