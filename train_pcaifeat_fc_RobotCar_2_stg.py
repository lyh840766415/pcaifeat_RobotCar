...
MODEL_PATH = '/home/lyh/lab/pcaifeat_RobotCar/model/2_stg_model_00939000/model_00939000.ckpt'
BATCH_NUM_QUERIES = 2
EPOCH = 100
...
def init_pcainetwork():
	...
	img_feat_reshape = tf.reshape(img_feat,[BATCH_NUM_QUERIES,(1+POSITIVES_PER_QUERY+NEGATIVES_PER_QUERY),img_feat.shape[1]])
	pc_feat_reshape = tf.reshape(pc_feat,[BATCH_NUM_QUERIES,(1+POSITIVES_PER_QUERY+NEGATIVES_PER_QUERY),pc_feat.shape[1]])
	q_img_vec, pos_img_vec, neg_img_vec = tf.split(img_feat_reshape, [1,POSITIVES_PER_QUERY,NEGATIVES_PER_QUERY],1)
	q_pc_vec, pos_pc_vec, neg_pc_vec = tf.split(pc_feat_reshape, [1,POSITIVES_PER_QUERY,NEGATIVES_PER_QUERY],1)
	...
	with tf.control_dependencies(update_ops):
		...
		train_img_op = optimizer.minimize(img_loss, global_step=batch)
		train_pc_op = optimizer.minimize(pc_loss, global_step=batch)
	...
	return images_placeholder,pc_placeholder,epoch_num_placeholder,all_loss,img_loss,pc_loss,train_op,train_img_op,train_pc_op,merged,batch
...
def main():
	images_placeholder,pc_placeholder,epoch_num_placeholder,all_loss,img_loss,pc_loss,train_op,train_img_op,train_pc_op,merged,batch = init_pcainetwork()
	...
	with tf.Session(config=config) as sess:
		...
		train_writer = tf.summary.FileWriter(os.path.join(LOG_DIR, 'train_save_2_stg'),sess.graph)
		...
		for ep in range(EPOCH):
			...
			for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
				...
				if i%3<2:
					summary,step,run_all_loss,run_img_loss,run_pc_loss,_,_,batch_num= sess.run([merged,batch,all_loss,img_loss,pc_loss,train_img_op,train_pc_op,batch],feed_dict = train_feed_dict)
				else:
					summary,step,run_all_loss,run_img_loss,run_pc_loss,_,batch_num= sess.run([merged,batch,all_loss,img_loss,pc_loss,train_op,batch],feed_dict = train_feed_dict)		
				...
				print("batch_num = %d , all_loss = %f"%(batch_num,run_all_loss))
				if step%3000 == 0:
					save_path = model_saver.save(sess, os.path.join(LOG_DIR,"train_save_2_stg", "model_%08d.ckpt"%(step)))
