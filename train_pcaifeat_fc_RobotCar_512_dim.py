...
MODEL_PATH = '/home/lyh/lab/pcaifeat_RobotCar/log/train_save_512/model_00900000.ckpt'
BATCH_NUM_QUERIES = 2
EPOCH = 100
...
def init_pcainetwork():
	...
	with tf.variable_scope("fusion_var"):
		...
		pcai_feat = tf.layers.dense(img_pc_concat_feat,512)
...
def main():
	...
	with tf.Session(config=config) as sess:
		...
		train_writer = tf.summary.FileWriter(os.path.join(LOG_DIR, 'train_save_512'),sess.graph)
		...
		for ep in range(EPOCH):
			...
			for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
				...
				if step%3000 == 0:
					save_path = model_saver.save(sess, os.path.join(LOG_DIR,"train_save_512", "model_%08d.ckpt"%(step)))
